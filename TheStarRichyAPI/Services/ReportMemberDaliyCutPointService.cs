...
{
    public interface IReportMemberDaliyCutPointService
    {
        Task<List<dynamic>> GetDisplayAsync();
    }
    public class ReportMemberDaliyCutPointService : IReportMemberDaliyCutPointService
    {
        private readonly IConfiguration _configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public ReportMemberDaliyCutPointService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<List<dynamic>> GetDisplayAsync()
        {
            ...
            try
            {
                using (var con = new SqlConnection(connectionString))
                {
                    ...
                    string query = "SELECT  calculatedate,Memberposition,LeftPV,RightPV,CutLeftPV,CutRightPV  ";
                    query += "  ,FWDLeftPV,FWDRightPV,StepPV,CalculatePV,weakBonus";
                    query += " FROM [000_Member_CutPoint_History]  (nolock) ";

                    query += " where Membercode = @Membercode  ";
                    query += " ORDER BY calculatedate desc  ";
