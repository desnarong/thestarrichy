...
namespace TheStarRichyApi.Services
{
    ...
    public class MemberDeliveryAddressService : IMemberDeliveryAddressService
    {
        private readonly IConfiguration _configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public MemberDeliveryAddressService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<List<dynamic>> GetDisplayAsync()
        {
            ...
            try
            {
                using (var con = new SqlConnection(connectionString))
                {
                    ...
                    string query = "SELECT  Membercode,Address,province,zipcode,M06_X11_HouseNumber,M06_X11_Alley  ";
                        query += " ,M06_X11_Road,M06_X11_Building,M06_X11_Floor,M06_X11_Other,M06_X11_Zipcode,M06_X11_TAMBON_ID";
                        query += " ,tambon,id,Expr1,amphoe,Expr2,Expr3,tambon_code ,amphoe_code,province_code ";
                    query += " From [000_Member_Delivery_address]  ";
      

                    query += " where Membercode = @Membercode  ";
                    //Contact_Primary 0-ที่อยู่หลัก มีได้อันเดียว  1-ที่อยู่อื่นๆ

                    query += " ORDER BY Membercode ";
