...
{
    public interface IFindRightBinaryService
    {
        Task<List<dynamic>> GetDisplayAsync();
    }
    public class FindRightBinaryService : IFindRightBinaryService
    {
        private readonly IConfiguration _configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public FindRightBinaryService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
            _configuration = configuration;
            _httpContextAccessor = httpContextAccessor;
        }
        public async Task<string> GetRightTeamAsync(string memberCode)
        {
            ...
            try
            {
                ...
                {
                    await con.OpenAsync();
                    string query = $"select Membercode,Member_1_2_R,Member_2_4_R ,Member_3_8_R  from MemberLevel  (nolock)  where Membercode=@Membercode";
                        ...
                        using (SqlDataReader reader = await command.ExecuteReaderAsync())
                        {
                            if (reader.HasRows)
                            {
                                while (await reader.ReadAsync())
                                {
                                    ...
                                    {
                                        Memberright = reader.GetString(3);
                                        Memberright = await GetRightTeamAsync(Memberright);
                                        return Memberright;
                                    }
                                    else
                                    if (!reader.IsDBNull(2))
                                    {
                                        Memberright = reader.GetString(2);
                                    }
                                    else
                                    if (!reader.IsDBNull(1))
                                    {
                                        Memberright = reader.GetString(1);
                                    }
                                    else
                                    { Memberright = reader.GetString(0); }
        ...
        public async Task<List<dynamic>> GetDisplayAsync()
        {
            ...
            try
            {
                using (var con = new SqlConnection(connectionString))
                {
                    ...
                    string Memberright = await GetRightTeamAsync(memberCode);


                    string query = "SELECT  M06_PX1 ";
                    query += " FROM [M06]   (nolock) ";


                    query += " where M06_PX1 = @Membercode  ";

                    using (var command = new SqlCommand(query, con))
                    {
                        command.Parameters.AddWithValue("@Membercode", Memberright);
