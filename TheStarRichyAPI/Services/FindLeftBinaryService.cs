...
{
    public interface IFindLeftBinaryService
    {
        Task<List<dynamic>> GetDisplayAsync();
    }
    public class FindLeftBinaryService : IFindLeftBinaryService
    {
        private readonly IConfiguration _configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public FindLeftBinaryService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
            _configuration = configuration;
            _httpContextAccessor = httpContextAccessor;
        }
        public async Task<string> GetLeftTeamAsync( string memberCode)
        {
            ...
            try
            {
                ...
                {
                    await con.OpenAsync();
                    string query = $"select Membercode,Member_1_1_L,Member_2_1_L ,Member_3_1_L  from MemberLevel  (nolock)  where Membercode=@Membercode";
                        ...
                        using (SqlDataReader reader = await command.ExecuteReaderAsync())
                        {
                            if (reader.HasRows)
                            {
                                while (await reader.ReadAsync())
                                {
                                    ...
                                    {
                                        Memberleft = reader.GetString(3);
                                        Memberleft = await GetLeftTeamAsync(Memberleft);
                                        return Memberleft;
                                    } 
                                    else
                                    if (!reader.IsDBNull(2))
                                    {  
                                        Memberleft = reader.GetString(2);
                                    }
                                    else
                                    if (!reader.IsDBNull(1))
                                    {
                                        Memberleft = reader.GetString(1);
                                    }
                                    else
                                    { Memberleft = reader.GetString(0); }
        ...
        public async Task<List<dynamic>> GetDisplayAsync()
        {
            ...
            try
            {
                using (var con = new SqlConnection(connectionString))
                {
                    ...
                    string Memberleft = await GetLeftTeamAsync( memberCode);
                    ...
                    {
                        command.Parameters.AddWithValue("@Membercode", Memberleft);
