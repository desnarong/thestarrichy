...
{
    public interface IReportMemberDailyPointService
    {
        Task<List<dynamic>> GetDisplayAsync();
    }
    public class ReportMemberDailyPointService : IReportMemberDailyPointService
    {
        private readonly IConfiguration _configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public ReportMemberDailyPointService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<List<dynamic>> GetDisplayAsync()
        {
            ...
            try
            {
                using (var con = new SqlConnection(connectionString))
                {
                    ...
                    string query = "SELECT  BalanceDate,LeftPV,RightPV ";
                    query += " FROM [000_Member_Daily_point]  (nolock) ";

                    query += " where Membercode = @Membercode ";
                    query += " ORDER BY BalanceDate desc  ";
