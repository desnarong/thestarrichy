...
{
    public interface IFindMemberNameService
    {
        Task<List<dynamic>> GetDisplayAsync();
    }
    public class FindMemberNameService : IFindMemberNameService
    {
        private readonly IConfiguration _configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public FindMemberNameService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<List<dynamic>> GetDisplayAsync()
        {
            ...
            string SearchDLname = _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            ...
            try
            {
                using (var con = new SqlConnection(connectionString))
                {
                    ...
                    string query = "SELECT  aa.Membercode, aa.DLcode,  aa.DLname ";
                    query += " FROM [000_Member_Binary_LeftRight_Search] aa (nolock) ";

                    if (Memberpermission != "Y")
                    {
                        query += "  join [000_Member_SponserTeam] bb  (nolock) on bb.Membercode=aa.Membercode and bb.DLCode=aa.DLcode  ";
                    }
                    query += " where aa.Membercode = @Membercode and aa.DLname=@SearchDLname";

                    using (var command = new SqlCommand(query, con))
                    {
                        command.Parameters.AddWithValue("@Membercode", memberCode);
                        command.Parameters.AddWithValue("@SearchDLname", SearchDLname);
