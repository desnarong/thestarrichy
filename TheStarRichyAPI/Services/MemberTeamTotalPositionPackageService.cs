...
{
    public interface IMemberTeamTotalPositionPackageService
    {
        Task<List<dynamic>> GetMember2DisplayAsync();
    }
    public class MemberTeamTotalPositionPackageService : IMemberTeamTotalPositionPackageService
    {
        private readonly IConfiguration _configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public MemberTeamTotalPositionPackageService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<List<dynamic>> GetMember2DisplayAsync()
        {
            ...
            try
            {
                ...
                {
                    await con.OpenAsync();
                    string query = "SELECT * FROM [000_Member_Team_total_position_package] WHERE Membercode = @Membercode ORDER BY M24_X8";
            ...
            {
                // Log exception (use your logging framework, e.g., Serilog, NLog)
                // _logger.LogError(ex, "Error fetching member data for Membercode: {Membercode}", memberCode);
                return new List<dynamic> { new { Membercode = "", Error = "An error occurred while fetching data" } };
            }

            // Return empty list if no rows, or the populated result
