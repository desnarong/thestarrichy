...
{
    public interface IFindUplineBinaryService
    {
        Task<List<dynamic>> GetDisplayAsync();
    }
    public class FindUplineBinaryService : IFindUplineBinaryService
    {
        private readonly IConfiguration _configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public FindUplineBinaryService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<List<dynamic>> GetDisplayAsync()
        {
            ...
            string memberCode = _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.NameIdentifier)?.Value;  //รหัสที่ต้องการค้นหา
            ...
            try
            {
                using (var con = new SqlConnection(connectionString))
                {
                    ...
                    string query = "SELECT   M06_X33  ";
                    query += " FROM [M06] aa (nolock) ";
 
                    query += " where aa.M06_PX1 = @memberCode";
