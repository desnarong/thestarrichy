...
{
    public interface IReportMemberPOOrderInvoiceDetailService
    {
        Task<dynamic> GetPOInvoiceDetailAsync(string billNo);
    }

    public class ReportMemberPOOrderInvoiceDetailService : IReportMemberPOOrderInvoiceDetailService
    {
        private readonly IConfiguration _configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public ReportMemberPOOrderInvoiceDetailService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<dynamic> GetPOInvoiceDetailAsync(string billNo)
        {
            ...
            try
            {
                using (var con = new SqlConnection(connectionString))
                {
                    ...
                    // Get header data from 000_Member_Order_PO
                    string headerQuery = @"SELECT * FROM [000_Member_Order_PO] (nolock)
                    ...
                    // Get detail data from 000_Member_Order_PO_detail
                    string detailQuery = @"SELECT * FROM [000_Member_Order_PO_detail] (nolock)
                                           WHERE BillNo = @BillNo
                                           ORDER BY No";
