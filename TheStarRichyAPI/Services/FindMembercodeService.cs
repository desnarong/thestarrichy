...
{
    public interface IFindMembercodeService
    {
        Task<List<dynamic>> GetDisplayAsync();
    }
    public class FindMembercodeService : IFindMembercodeService
    {
        private readonly IConfiguration _configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public FindMembercodeService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<List<dynamic>> GetDisplayAsync()
        {
            ...
            string SearchDLcode = _httpContextAccessor.HttpContext.User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
            ...
            try
            {
                using (var con = new SqlConnection(connectionString))
                {
                    ...
                    query += " FROM [000_Member_Binary_LeftRight_Search1] aa (nolock) ";
                    ...
                    query += " ORDER BY aa.Item,aa.MemberLeftCode ";

                    using (var command = new SqlCommand(query, con))
                    {
                        command.Parameters.AddWithValue("@Membercode", memberCode);
                        command.Parameters.AddWithValue("@SearchDLcode", SearchDLcode);
