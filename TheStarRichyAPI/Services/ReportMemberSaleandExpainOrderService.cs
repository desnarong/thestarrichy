...
namespace TheStarRichyApi.Services
{
    ...
    public class ReportMemberSaleandExpainOrderService : IReportMemberSaleandExpainOrderService
    {
        ...
        public ReportMemberSaleandExpainOrderService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
        ...
        public async Task<List<dynamic>> GetDisplayAsync(string fromDate = "", string toDate = "")
        {
            ...
            try
            {
                using (var con = new SqlConnection(connectionString))
                {
                    ...
                    string query = "SELECT * FROM [000_Member_Order_Center_for_mobile] (nolock) ";
                    query += " WHERE (Createby = @Membercode OR HybridGOLDID = @Membercode OR MobileID = @Membercode OR CenterID = @Membercode) ";
