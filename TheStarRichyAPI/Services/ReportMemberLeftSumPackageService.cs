...
{
    public interface IReportMemberLeftSumPackageService
    {
        Task<List<dynamic>> GetDisplayAsync();
    }
    public class ReportMemberLeftSumPackageService : IReportMemberLeftSumPackageService
    {
        private readonly IConfiguration _configuration;
        private readonly IHttpContextAccessor _httpContextAccessor;

        public ReportMemberLeftSumPackageService(IConfiguration configuration, IHttpContextAccessor httpContextAccessor)
        {
            ...
            try
            {
                using (var con = new SqlConnection(connectionString))
                {
                    ...
                    string query = "SELECT  Membercode,  PositionName  ,Totalmember" +
                        " FROM [000_Member_Left_Sum_position_package] where Membercode=@Membercode ORDER BY M24_X8 ";
